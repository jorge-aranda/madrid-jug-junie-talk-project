...
class RemoveTaskFromGroupUseCaseTest {

    private val taskGroupService: TaskGroupService = mockk()
    private val useCase = RemoveTaskFromGroupUseCase(taskGroupService = taskGroupService)

    @Test
    fun `should remove a task from a group owned by the user`() {
        // given
        val userId = "user-1"
        val taskId = "task-1"
        val taskGroup = TaskGroupMother.withTasks(userId = userId, taskIds = listOf(taskId))
        every { taskGroupService.findById(id = taskGroup.id) } returns taskGroup
        every {
            taskGroupService.removeTask(taskGroup = taskGroup, taskId = taskId)
        } returns taskGroup.copy(taskIds = emptyList())

        // when
        val result = useCase.execute(taskGroupId = taskGroup.id, taskId = taskId, userId = userId)

        // then
        assertTrue(result.taskIds.isEmpty())
    }
